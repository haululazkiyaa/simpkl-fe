...
export default function StudentDataTableView(props) {
  ...
  const handleStatusSiswa = () => {
    ...
    refreshToken((status, token) => {
      if (status) {
        setProgress(60);
        setSiswa(requestData, token, (status) => {
          if (status) {
            toast.success(
  ...
  const handleDeleteSiswa = () => {
    ...
    refreshToken((status, token) => {
      if (status) {
        setProgress(60);
        deleteSiswa(requestData, token, (status, message) => {
          if (status) {
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor((row) => `${row.nis ?? "-"} / ${row.nisn ?? "-"}`, {
        id: "nis_nisn",
        header: "NIS / NISN",
        enableSorting: false,
      }),
      ...
      columnHelper.accessor("jurusan.kompetensi_keahlian", {
        header: "Jurusan",
        cell: (info) => (
          <div className="text-left truncate max-w-xs">
            {info.getValue() ?? "-"}
      ...
      <div className="mb-4 flex flex-col md:flex-row justify-between items-start md:items-center">
        <div className="relative w-full md:w-96 mb-4 md:mb-0">
          ...
          <input
            ...
            placeholder="Cari nama siswa, jurusan, alamat..."
            value={searchTerm}
