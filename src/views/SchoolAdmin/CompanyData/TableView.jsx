...
export default function CompanyDataTableView(props) {
  ...
  const handleStatusPerusahaan = (option) => {
    ...
    refreshToken((status, token) => {
      if (status) {
        setProgress(60);
        setPerusahaan(requestData, token, (status, message) => {
          if (status) {
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor("id", {
        id: "foto",
        header: "Foto Perusahaan",
        cell: (info) => (
          <Button
            variant="default"
            onClick={() => initStaticModal(info.row.original)}
          >
      ...
      columnHelper.accessor("alamat", {
        header: "Alamat",
        cell: (info) => (
          <div className="text-left min-w-[300px]">{info.getValue()}</div>
        ),
      }),
      ...
      columnHelper.accessor("user.temp_password", {
        header: "Kata Sandi Sementara",
        cell: (info) => <div>{info.getValue() || "-"}</div>,
      }),
      ...
      columnHelper.accessor("id", {
        ...
        cell: (info) => {
          ...
          return (
            (item.status === "AKTIF" || item.status === "NONAKTIF") && (
              <div className="flex items-center justify-center">
      ...
      columnHelper.accessor("id", {
        id: "approval",
        header: "Approval Akun",
        cell: (info) => {
          const item = info.row.original;
          return item.status === "PENDING" ? (
            <div className="flex items-center justify-center space-x-2">
              <Button variant="green" onClick={() => initModal1(item)}>
      ...
      <div className="mb-4 flex flex-col md:flex-row justify-between items-start md:items-center">
        <div className="relative w-full md:w-96 mb-4 md:mb-0">
          ...
          <input
            ...
            placeholder="Cari perusahaan, email, username..."
            value={searchTerm}
              ...
              <tr key={headerGroup.id}>
                {headerGroup.headers.map((header) => (
                  <th
                    ...
                    className={`px-6 py-3 ${
                      header.id === "no" ||
                      ["aktif", "approval", "edit", "foto"].includes(header.id)
                        ? "w-16 px-3"
                        : ""
                    }`}
                ...
                >
                  {row.getVisibleCells().map((cell) => (
                    <td
                      ...
                      className={`px-6 py-4 ${
                        cell.column.id === "no" ||
                        ["aktif", "approval", "edit", "foto"].includes(
                          cell.column.id
                        )
                          ? "w-16 px-3"
                          : ""
                      } ${cell.column.id === "alamat" ? "min-w-[300px]" : ""}`}
                    >
                      {flexRender(
