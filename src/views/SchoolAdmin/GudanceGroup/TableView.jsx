...
export default function GuidanceGroupTableView(props) {
  ...
  const handleStatusKelBimbingan = () => {
    ...
    refreshToken((status, token) => {
      if (status) {
        setProgress(60);
        updateKelBimbingan(requestData, token, (status) => {
          if (status) {
            toast.success(
  ...
  const handleDeleteKelBimbingan = () => {
    ...
    refreshToken((status, token) => {
      if (status) {
        setProgress(60);
        deleteKelBimbingan(requestData, token, (status, message) => {
          if (status) {
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor(
        (row) => `${row.siswa?.nis ?? "-"} / ${row.siswa?.nisn ?? "-"}`,
        {
          id: "nis_nisn",
          header: "NIS / NISN",
          enableSorting: false,
      ...
      columnHelper.accessor("siswa.nama", {
        header: "Nama Siswa",
        cell: (info) => (
      ...
      columnHelper.accessor("guru_pembimbing.nama", {
      ...
      columnHelper.accessor("perusahaan.nama_perusahaan", {
        header: "Perusahaan",
        cell: (info) => (
          <div className="truncate max-w-xs">{info.getValue() ?? "-"}</div>
        ),
      }),
      columnHelper.accessor("instruktur.nama", {
        header: "Instruktur",
        cell: (info) => (
          <div className="text-left truncate max-w-xs">
            {info.getValue() ?? "-"}
      ...
      columnHelper.accessor("id", {
        id: "aktif",
        header: "Aktif/ Non-aktifkan",
        cell: (info) => (
          <div className="flex items-center justify-center">
            <Button
              variant="default"
              onClick={() => initModal(info.row.original)}
            >
      ...
      columnHelper.accessor("id", {
        ...
        cell: (info) => (
          <div className="flex items-center justify-center">
            <Button variant="red" onClick={() => initModal1(info.row.original)}>
      ...
      <div className="mb-4 flex flex-col md:flex-row justify-between items-start md:items-center">
        <div className="relative w-full md:w-96 mb-4 md:mb-0">
          ...
          <input
            ...
            placeholder="Cari data siswa, guru, instruktur..."
            value={searchTerm}
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <tr key={headerGroup.id}>
                {headerGroup.headers.map((header) => (
                  <th
                    key={header.id}
                    className={`px-6 py-3 ${header.id === "no" ? "w-16" : ""} ${
                      ["aktif", "edit", "delete"].includes(header.id)
                        ? "w-16 px-3"
                        : ""
                    }`}
                ...
                >
                  {row.getVisibleCells().map((cell) => (
                    <td
                      key={cell.id}
                      className={`px-6 py-4 ${
                        cell.column.id === "no" ? "w-16" : ""
                      } ${
                        ["aktif", "edit", "delete"].includes(cell.column.id)
                          ? "w-16 px-3"
                          : ""
                      }`}
                    >
